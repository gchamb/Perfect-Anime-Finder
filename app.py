...
def displayAnime():
    ...
    for i in range(0,len(getAnimes)):
        for j in range(0, len(getAnimes[i]['genres'])):
            if getAnimes[i]['genres'][j]['name'] == genre_two:
                storeOneTwo.append(getAnimes[i])

    for i in range(0,len(storeOneTwo)):
        for j in range(0, len(storeOneTwo[i]['genres'])):   
            # Checks for third genre 
            if storeOneTwo[i]['genres'][j]['name'] == genre_three:   
    ...
    # Makes the data managable without using the JikanApi again since they have API Limit
    ...
    referenceDict = animeInfo # References the dictionary so other routes can use the data
